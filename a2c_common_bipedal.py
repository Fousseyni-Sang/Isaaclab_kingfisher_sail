...
class ReplayBuffer:
    ...
    def sample_buffer(self, num_envs, batch_size):
        max_mem = self.mem_size # min(self.mem_cntr, self.mem_size)
        batch = torch.randint(
...
class DiscriminatorNetwork(nn.Module):
    def __init__(self, lr, state_dim, context_dim,
        ...
        self.loss_spread = torch.zeros(1, device=self.device)
        self.loss_mse = torch.zeros(1, device=self.device)
        self.loss_spearman = torch.zeros(1, device=self.device)
    ...
    def predict(self, state, reparameterize=True, requires_grad=True):
        if requires_grad:
            ...
            samples = torch.clamp(samples, 0.000001, 0.99999999)
    ...
    def learn(self, state_batch, context_batch):
        ...
        spear_loss =  200*(1 - spear_corr).detach() #200*(1 - spear_corr)
        #neg_log_loss = -log_probs.mean()

        mse = 500*F.mse_loss(predictions, context_batch)
        spread = 100*(1 / torch.abs(torch.min(dist.loc) - torch.max(dist.loc)))
        loss = (mse) + spread #+ spear_loss
        ...
        self.loss = loss
        self.loss_mse = mse
        self.loss_spearman = spear_loss
        self.loss_spread = spread
...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        self.discriminator1 = DiscriminatorNetwork(lr=0.0001, state_dim=1, fc1_dims=256, fc2_dims=256, context_dim=1, 
                                                   num_envs=self.num_actors, device=self.device)
        self.discriminator2 = DiscriminatorNetwork(lr=0.0001, state_dim=1, fc1_dims=256, fc2_dims=256, context_dim=1, 
                                                   num_envs=self.num_actors, device=self.device)
        self.discriminator3 = DiscriminatorNetwork(lr=0.0001, state_dim=1, fc1_dims=256, fc2_dims=256, context_dim=1, 
                                                   num_envs=self.num_actors, device=self.device)
        
        self.spear_loss1 = torch.zeros(1, device=self.device)
        self.spear_loss2 = torch.zeros(1, device=self.device)
        self.spear_loss3 = torch.zeros(1, device=self.device)
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, c_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames):
        ...
        self.writer.add_scalar('losses/disc1_loss', torch_ext.mean_list(self.discriminator1.losses).item(), frame) # My added line
        self.writer.add_scalar('losses/disc1_reward', self.discriminator1.rewards.item(), frame) # My added line
        self.writer.add_scalar('losses/disc1_loss_mse', self.discriminator1.loss_mse.item(), frame) # My added line
        self.writer.add_scalar('losses/disc1_loss_spread', self.discriminator1.loss_spread.item(), frame) # My added line
        self.writer.add_scalar('losses/disc1_loss_spearman', self.discriminator1.loss_spearman.item(), frame) # My added line
        
        self.writer.add_scalar('losses/disc2_loss', torch_ext.mean_list(self.discriminator2.losses).item(), frame) # My added line
        self.writer.add_scalar('losses/disc2_reward', self.discriminator2.rewards.item(), frame) # My added line
        self.writer.add_scalar('losses/disc2_loss_mse', self.discriminator2.loss_mse.item(), frame) # My added line
        self.writer.add_scalar('losses/disc2_loss_spread', self.discriminator2.loss_spread.item(), frame) # My added line
        self.writer.add_scalar('losses/disc2_loss_spearman', self.discriminator2.loss_spearman.item(), frame) # My added line

        self.writer.add_scalar('losses/disc3_loss', torch_ext.mean_list(self.discriminator3.losses).item(), frame) # My added line
        self.writer.add_scalar('losses/disc3_reward', self.discriminator3.rewards.item(), frame) # My added line
        self.writer.add_scalar('losses/disc3_loss_mse', self.discriminator3.loss_mse.item(), frame) # My added line
        self.writer.add_scalar('losses/disc3_loss_spread', self.discriminator3.loss_spread.item(), frame) # My added line
        self.writer.add_scalar('losses/disc3_loss_spearman', self.discriminator3.loss_spearman.item(), frame) # My added line

        self.writer.add_scalar('losses/loss_spearman1', self.spear_loss1.item(), frame) # My added line
        self.writer.add_scalar('losses/loss_spearman2', self.spear_loss2.item(), frame) # My added line
        self.writer.add_scalar('losses/loss_spearman3', self.spear_loss3.item(), frame) # My added line
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            acord_reward_scale = 0.2
            discriminator1_states = self.obs["obs"][:, 0:1] #for kingfisher_sail  self.obs["obs"][:, 7:8] #
            discriminator1_context = self.obs["obs"][:, -1:] 
            self.discriminator1.memory.store_transition(discriminator1_states, discriminator1_context)
            prediction1, log_probs1, distrib1 = self.discriminator1.predict(discriminator1_states, reparameterize=False, requires_grad=False)
            prediction_error1 = torch.clamp(torch.abs(prediction1-discriminator1_context)**2, min=0.001, max=0.99999)
            reward_disc1 = acord_reward_scale * (- torch.log(prediction_error1))
            self.discriminator1.rewards = reward_disc1.mean()
            

            discriminator2_states = self.obs["obs"][:, 2:3] #for kingfisher_sail  self.obs["obs"][:, 7:8] #
            discriminator2_context = self.obs["obs"][:, -2:-1] 
            #self.discriminator2.memory.store_transition(discriminator2_states, discriminator2_context)
            prediction2, log_probs2, distrib2 = self.discriminator2.predict(discriminator2_states, reparameterize=False, requires_grad=False)
            prediction_error2 = torch.clamp(torch.abs(prediction2-discriminator2_context)**2, min=0.001, max=0.99999)
            reward_disc2 = acord_reward_scale * (- torch.log(prediction_error2))
            self.discriminator2.rewards = reward_disc2.mean()

            discriminator3_states = self.obs["obs"][:, 4:5] #for kingfisher_sail  self.obs["obs"][:, 7:8] #
            discriminator3_context = self.obs["obs"][:, -3:-2] 
            self.discriminator3.memory.store_transition(discriminator3_states, discriminator3_context)
            prediction3, log_probs3, distrib3 = self.discriminator3.predict(discriminator3_states, reparameterize=False, requires_grad=False)
            prediction_error3 = torch.clamp(torch.abs(prediction3-discriminator3_context)**2, min=0.001, max=0.99999)
            reward_disc3 = acord_reward_scale * (- torch.log(prediction_error3))
            self.discriminator3.rewards = reward_disc3.mean()

            reward_acord = (reward_disc1 + reward_disc2 + reward_disc3)/3
            rewards = 0.5*rewards + reward_acord
...
class ContinuousA2CBase(A2CBase):
    ...
    def train_epoch(self):
        ...
        kls = []
        disc1_losses = []
        self.discriminator1.losses = []
        disc1_mean_mse_loss = 0
        disc1_mean_spearman_loss = 0
        disc1_mean_spread_loss = 0

        disc2_losses = []
        self.discriminator2.losses = []
        disc2_mean_mse_loss = 0
        disc2_mean_spearman_loss = 0
        disc2_mean_spread_loss = 0

        disc3_losses = []
        self.discriminator3.losses = []
        disc3_mean_mse_loss = 0
        disc3_mean_spearman_loss = 0
        disc3_mean_spread_loss = 0
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.dataset)):
                ...
                disc1_loss = self.discriminator1.loss
                disc2_loss = self.discriminator2.loss
                disc3_loss = self.discriminator3.loss
                ...
                disc1_mean_mse_loss += self.discriminator1.loss_mse
                disc1_mean_spearman_loss += self.discriminator1.loss_spearman
                disc1_mean_spread_loss += self.discriminator1.loss_spread

                disc2_losses.append(disc2_loss)
                disc2_mean_mse_loss += self.discriminator2.loss_mse
                disc2_mean_spearman_loss += self.discriminator2.loss_spearman
                disc2_mean_spread_loss += self.discriminator2.loss_spread

                disc3_losses.append(disc3_loss)
                disc3_mean_mse_loss += self.discriminator3.loss_mse
                disc3_mean_spearman_loss += self.discriminator3.loss_spearman
                disc3_mean_spread_loss += self.discriminator3.loss_spread
        ...
        self.discriminator1.losses = disc1_losses
        self.discriminator1.loss_mse = disc1_mean_mse_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator1.loss_spearman = disc1_mean_spearman_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator1.loss_spread = disc1_mean_spread_loss / (self.mini_epochs_num * len(self.dataset))

        self.discriminator2.losses = disc2_losses
        self.discriminator2.loss_mse = disc2_mean_mse_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator2.loss_spearman = disc2_mean_spearman_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator2.loss_spread = disc2_mean_spread_loss / (self.mini_epochs_num * len(self.dataset))

        self.discriminator3.losses = disc3_losses
        self.discriminator3.loss_mse = disc3_mean_mse_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator3.loss_spearman = disc3_mean_spearman_loss / (self.mini_epochs_num * len(self.dataset))
        self.discriminator3.loss_spread = disc3_mean_spread_loss / (self.mini_epochs_num * len(self.dataset))
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul

    def prepare_dataset(self, batch_dict):
        obses = batch_dict['obses']
        #print(f"hey, context: {obses[:, 15]} energy: {obses[:, 14]}")
        returns = batch_dict['returns']
        dones = batch_dict['dones']
        values = batch_dict['values']
        actions = batch_dict['actions']
        ...
        #print(f"returns: {returns} sum_returns: {torch.mean(returns)}")
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if self.save_freq > 0:
                        if epoch_num % self.save_freq == 0:
                            ...
                            acord_dir = self.experiment_dir.split("/")
                            acord_dir_name = ('/'.join(acord_dir[:-3]) + "/acord/" + '/'.join(acord_dir[-2:])) if \
                            "runs" not in acord_dir else acord_dir[-2] + "/acord/"+acord_dir[-1]

                            checkpoint_name_disc1 = acord_dir_name + "/nn/" 
                            checkpoint_name_disc2 = acord_dir_name + "/nn2/"
                            checkpoint_name_disc3 = acord_dir_name + "/nn3/"
                            
                            self.discriminator1.save(checkpoint_name_disc1, step_marker=str(mean_rewards[0]))
                            self.discriminator2.save(checkpoint_name_disc2, step_marker=str(mean_rewards[0]))
                            self.discriminator3.save(checkpoint_name_disc3, step_marker=str(mean_rewards[0]))
