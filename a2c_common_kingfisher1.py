...
class DiscriminatorNetwork(nn.Module):
    ...
    def learn(self, state_batch, context_batch):
        ...
        spread = 50*(1 / torch.abs(torch.min(dist.loc) - torch.max(dist.loc)))
...
class A2CBase(BaseAlgorithm):
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            reward_disc2 = 0*120*acord_reward_scale * (- torch.log(prediction_error2))
            ...
            reward_disc3 = 0*acord_reward_scale * (- torch.log(prediction_error3))
            self.discriminator3.rewards = reward_disc3.mean()"""

            reward_acord = reward_disc1 #(reward_disc1 + reward_disc2 + reward_disc3)/3
            rewards = rewards + reward_acord
