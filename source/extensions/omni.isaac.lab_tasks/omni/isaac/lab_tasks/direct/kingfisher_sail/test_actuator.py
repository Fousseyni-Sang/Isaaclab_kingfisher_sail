...
actuator_cfg.precision = 0.01  # radians
actuator_cfg.scale_joint_pos = torch.pi/6  # radians per command unit
...
iteration = 0
while rudder_actuator.current_cmd.abs() < target_cmds.abs():
    rudder_actuator.update_joint_cmd(target_cmds)
...
plt.hlines(y=target_cmds.item(), xmin=0*target_cmds, xmax=len(cmd_list), color='r', linestyle='--', label='Target cmd' )
