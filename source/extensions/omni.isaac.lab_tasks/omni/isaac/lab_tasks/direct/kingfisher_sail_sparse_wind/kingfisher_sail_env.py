...
class KingfisherSailEnvCfg(DirectRLEnvCfg):
    ...
    observation_space = gym.spaces.Dict({
                        ...
                        "achieved_goal": gym.spaces.Box(low=np.array([-np.inf, -np.inf, -np.pi, -np.inf]), high=np.array([np.inf, np.inf, np.pi, np.inf]), shape=(4, ), dtype=np.float32), # [x, y, theta, energy]
                        "desired_goal": gym.spaces.Box(low=np.array([-np.inf, -np.inf, -np.pi, -np.inf]), high=np.array([np.inf, np.inf, np.pi, np.inf]), shape=(4, ), dtype=np.float32), # [x, y, theta, energy]
                        #"achieved_goal": gym.spaces.Box(low=np.array([-np.inf, -np.inf, -np.pi]), high=np.array([np.inf, np.inf, np.pi]), shape=(4, ), dtype=np.float32), # [x, y, theta]
...
class KingfisherSailEnv(DirectRLEnv):
    ...
    def compute_reward(self, achieved_goal, desired_goal, info={}, is_her=True):
        ...
        if isinstance(achieved_goal, np.ndarray):
            ...
            pos_error = np.linalg.norm(2*achieved_goal[:, :2] - desired_goal[:, :2], axis=1)
            ...
            energy_penalty = -(1-alpha) * achieved_goal[:, 3] # Assuming energy usage is the 4th element of the achieved_goal
            return -alpha*norm_pos_error + success + energy_penalty
        ...
        # Case 2: IsaacLab internal reward → Torch tensors
            ...
            ag = achieved_goal
            dg = desired_goal
            pos_error = torch.norm(2*ag[:, :2] - dg[:, :2], dim=1)
            ...
            energy_penalty = -(1-alpha) * achieved_goal[:, 3]
            return -alpha*norm_pos_error, success, energy_penalty
        else:
    ...
    def _get_observations(self) -> dict:
        ...
        obs = {
            ...
            "achieved_goal": torch.cat([current_config, self.normalized_energy.clone().reshape(-1, 1)], 
            dim=1), # 3 # energy is added to the achieved goal for HER training with energy optimization
            "desired_goal":torch.cat([target_config, self.normalized_energy.clone().reshape(-1, 1)], 
            dim=1), # 3
        }
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        aumented_current_config = torch.cat([current_config, self.normalized_energy.clone().reshape(-1, 1)], dim=1)
        aumented_target_config = torch.cat([target_config, self.normalized_energy.clone().reshape(-1, 1)], dim=1)
        reward_progress, goal_reward, energy_reward = self.compute_reward(aumented_current_config, aumented_target_config)

        self.reward_progress = reward_progress.clone()
        self.reward_success = goal_reward.clone()
        #self.reward_time[:] = energy_reward
        self.energy_reward = energy_reward.clone()

        rewards = {  
            "2_goal_reached": self.reward_success,
            "1_distance_progress":self.reward_progress,
            "3_energy": self.energy_reward,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if self.is_Training:
            ...
            self._sail_aerodynamics.reset_flow_condition(env_ids=env_ids, randomize_direction=False, randomize_speed=False, 
                                                    upflow=True, downflow=True, beam=True, close=True, broad=True,
                                                    fixed=True)
