...
class Se3GamepadKingfisher(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    def __init__(self, pos_sensitivity: float = 1.0, rot_sensitivity: float = 1, dead_zone: float = 0.01):
        ...
        self._delta_pose_raw = np.zeros([2, 7]) #np.zeros([2, 6])
    ...
    def advance(self) -> tuple[np.ndarray, list[bool]]:
        ...
        sail_angle = self._resolve_command_buffer(self._delta_pose_raw[:, 6:])
        # -- convert to rotation vector
        rot_vec = Rotation.from_euler("XYZ", delta_rot).as_rotvec()
        # return the command and gripper state
        return np.concatenate([delta_pos, rot_vec, sail_angle]), self._close_gripper
    ...
    def _on_gamepad_event(self, event, *args, **kwargs):
        ...
        # ============================================================== 
        # 
        if event.input in self._INPUT_TRIGGER_VALUE_MAPPING:
            direction, axis, value = self._INPUT_TRIGGER_VALUE_MAPPING[event.input]
            # change the value only if the stick is moved (soft press)
            self._delta_pose_raw[direction, axis] = value * cur_val
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_TRIGGER_VALUE_MAPPING = {
            carb.input.GamepadInput.LEFT_TRIGGER: (1, 6, self.pos_sensitivity),
            carb.input.GamepadInput.RIGHT_TRIGGER: (0, 6, self.pos_sensitivity),
        }
        # ==============================================================
