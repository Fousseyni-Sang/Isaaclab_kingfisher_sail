...
home_dir = os.environ.get("HOME_DIREC", "/mnt/gpu_storage/zrr")
...
class FoilDynamics:
    def __init__(self, num_envs, device, cfg: FoilDynamicsCfg, naca_profile: str = "0018"):
    ...
    def get_max_aero_force(self, true_wind, max_boat_speed=1.5) -> torch.Tensor:
        ...
        """
        if isinstance(1.0*true_wind, float):
    ...
    def init_flow_vector(self, ship_vel_2D: torch.Tensor, ship_heading_w:torch.Tensor):
        """
            This function initializes the wind vector in the robot frame
            Parameters:
                - env_ids: torch.Tensor | None = None : indices of the environments to initialize the wind vector
            Return:
                - wind_vector_w: torch.Tensor of shape (num_envs, 3) : wind vector in the world frame
        """

        # True flow in foilboat frame
        self.true_flow_speed2D_b = self.generate_true_flow_components(self.Uw, self.Beta_w, ship_heading_w) # ok

        apparent_flow2D_b = self.generate_apparent_flow_components(self.true_flow_speed2D_b, ship_vel_2D)
