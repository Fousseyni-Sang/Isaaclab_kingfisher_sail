...
def debug_vis_callback(robot, sail_aerodynamics, blue_marker, red_marker, marker_translations, red_marker_translations, 
    ...
    #print(f"{thrust_left.shape} -> {thrust_right.shape} -> {sail_aerodynamic_force_b.squeeze(0)[:, :2].shape}")
...
class SailPublisher(rclpy.node.Node):
    def __init__(self):
        ...
        self.apparent_flow_angle_publisher = self.create_publisher(Float32, "apparent_flow_angle", 10)
    ...
    def publish(self, current_pos, target_pos, actual_pos, cl, cd, aoa, app_flow_angle, robot_pos, robot_vel, aero_force, 
        ...
        self.apparent_flow_angle_publisher.publish(msg_app_flow_angle)  # Publish apparent wind angle
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        rew_aero = torch.sum(sail_aerodynamic_force_b[:, 0, :2] * desired_pos_b_3d[:, :2], dim=-1) / (max_aero_force*distance)
