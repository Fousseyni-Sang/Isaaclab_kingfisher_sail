...
class HRLA2CBuilder(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            self.actor_mlp = nn.Sequential()
            self.critic_mlp = nn.Sequential()
            self.feas_encoder = nn.Sequential(
                nn.Conv2d(self.hrl_embed_in, 16, 3, stride=2, padding=1),
                nn.ReLU(),
                ...
                # <--- This makes the network robust to ANY input size
                nn.AdaptiveAvgPool2d((2, 2)),

                nn.Flatten(),
                nn.Linear(64, self.hrl_embed_out),
                nn.ReLU(),
            )
            ...
            mlp_args = {
                'input_size' : in_mlp_shape+self.hrl_embed_out, # add the embedding (output of self.feas_encoder) size to it
                'units' : self.units, 
                'activation' : self.activation, 
                'norm_func_name' : self.normalization,
                'dense_func' : torch.nn.Linear,
        ...
        def forward(self, obs_dict):
            ...
            out = obs
            out = out.flatten(1)   
            feas = obs_dict['feasibility_map']
            
            z = self.feas_encoder(feas) 
        ...
        def load(self, params):
            ...
            self.central_value = params.get('central_value', False)
            self.joint_obs_actions_config = params.get('joint_obs_actions', None)
            self.hrl_embed_in = params['hrl_embbeding']['in_size']
            self.hrl_embed_out = params['hrl_embbeding']['out_size']
            if self.has_space:
                self.is_multi_discrete = 'multi_discrete'in params['space']
                self.is_discrete = 'discrete' in params['space']
                self.is_continuous = 'continuous'in params['space']
    ...
    def build(self, name, **kwargs):
        net = HRLA2CBuilder.Network(self.params, **kwargs)
        return net
...
class HRLSACBuilder(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
    ...
    def build(self, name, **kwargs):
        net = HRLSACBuilder.Network(self.params, **kwargs)
        return net
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            actor_mlp_args = {
                'input_size' : obs_dim + self.hrl_embed_out, 
                'units' : self.units, 
                'activation' : self.activation, 
                'norm_func_name' : self.normalization,
                'dense_func' : torch.nn.Linear,
            ...
            critic_mlp_args = {
                'input_size' : obs_dim + action_dim + self.hrl_embed_out, 
                'units' : self.units, 
                'activation' : self.activation, 
                'norm_func_name' : self.normalization,
                'dense_func' : torch.nn.Linear,
