...
class A2CBase(BaseAlgorithm):
    ...
    def get_action_values(self, obs):
        ...
        feas_map = obs.get("feasibility_map", None)
        if feas_map is not None:
            input_dict.update({
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            loss_mask = infos.get('loss_mask', None)
            feas_map = self.obs.get('feasibility_map', None)
            
            if feas_map is not None:
                self.experience_buffer.update_data('feasibility_map', n, feas_map)
                
            if loss_mask is not None:
                self.experience_buffer.update_data('loss_mask', n, loss_mask)
        ...
        if loss_mask is not None:
            mb_advs = self.discount_values_masks(fdones, last_values, mb_fdones, mb_values, mb_rewards, loss_mask)
        else:
            mb_advs = self.discount_values(fdones, last_values, mb_fdones, mb_values, mb_rewards)
        
        mb_returns = mb_advs + mb_values
        
        if loss_mask is None and "loss_mask" in self.tensor_list:
            self.tensor_list.remove("loss_mask")

        if feas_map is None and "feasibility_map" in self.tensor_list:
            self.tensor_list.remove("feasibility_map")
...
class ContinuousA2CBase(A2CBase):
    ...
    def init_tensors(self):
        ...
        self.tensor_list = self.update_list + ['obses', 'states', 'dones', 'loss_mask', 'feasibility_map']
    ...
    def prepare_dataset(self, batch_dict):
        ...
        loss_mask = batch_dict.get('loss_mask', None)
        feas_map = batch_dict.get('feasibility_map', None)
        ...
        dataset_dict = {}
        #print(f"loss: {loss_mask.shape, neglogpacs.shape, (neglogpacs*loss_mask).shape}")
        dataset_dict['old_values'] = values 
        dataset_dict['old_logp_actions'] = neglogpacs 
        dataset_dict['advantages'] = advantages 
        dataset_dict['returns'] = returns 
        ...
        if loss_mask is not None:
            dataset_dict['loss_mask'] = loss_mask
        if feas_map is not None:
            dataset_dict['feasibility_map'] = feas_map
        
        #print(f"non zero stuff: {loss_mask.count_nonzero(), advantages.count_nonzero()}")
        self.dataset.update_values_dict(dataset_dict)
