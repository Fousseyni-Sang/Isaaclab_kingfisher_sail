    ...
    while simulation_app.is_running() and episodes_finished < num_episodes_target:
        ...
        with torch.inference_mode():
            ...
            if actions.shape[1]<3:
                actions = torch.cat([actions, torch.zeros((num_envs, 3-actions.shape[1]), device=rew.device)], dim=-1)

            robot_pos = extras["info"].get("robot_pos_w", torch.zeros((num_envs, 2), device=rew.device))[..., :2] # (N, 2) 
            ...
            true_wind_angle_b = extras.get("info", {}).get("true_wind_angle_b", torch.zeros((num_envs, ), device=rew.device)) # (N,) 
            true_wind_angle_w = extras.get("info", {}).get("true_wind_angle_w", torch.zeros((num_envs, ), device=rew.device)) # (N,) 
            ...
            norm_error_lin = extras["info"].get("norm_error_lin", torch.zeros((num_envs,2), device=rew.device)) # (N,) 
            norm_error_ang = extras["info"].get("norm_error_ang", torch.zeros((num_envs,), device=rew.device)) # (N,) 
            goal_pos = extras["info"].get("goal_pos", torch.zeros((num_envs, 2), device=rew.device)) # (N, 2) 
            ...
            if args_cli.ros: #  and (step_idx % args_cli.ros_publish_interval == 0): 
                dyn_ros_node.publish( 
                    ...
                    true_flow_angle_b=true_wind_angle_b, 
                    true_flow_angle_w=true_wind_angle_w,
            ...
            for env_id in range(num_envs): 
                step_record = { 
                    ...
                    "true_wind_angle_w": true_wind_angle_w[env_id].item(), 
