...
parser.add_argument("--output_csv", type=str, default="all_policies_swept.csv",
                     help="CSV path. Runs APPEND to this file instead of overwriting it, so "
                          "multiple policies (contextualized + fixed-budget baselines) can be "
                          "played into the same file for a single combined plot.")
...
def save_append(df: pd.DataFrame, csv_path: str):
    """Append df to csv_path, writing the header only if the file doesn't
    exist yet. This is what lets multiple separate runs (one per policy)
    accumulate into a single combined file instead of each overwriting the
    last one."""
    file_exists = os.path.exists(csv_path)
    df.to_csv(csv_path, mode="a", header=not file_exists, index=False)
    action = "Appended to" if file_exists else "Created"
    print(f"[INFO] {action} {csv_path} ({len(df)} rows this run)")
...
def main():
    ...
    try:
        ...
        save_append(df, args_cli.output_csv)
        print(f"[INFO] This run: {len(df)} rows, "
            f"{df.groupby(['env_id','local_episode_idx']).ngroups} episodes.")
