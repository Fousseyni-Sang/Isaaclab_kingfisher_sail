...
df_norm_error = pd.read_csv(os.path.join(latest_dir, "norm_error.csv"))
...
plt.quiver(15, 0, wind_x, wind_y, angles='xy', scale_units='xy', scale=0.5, color='blue', label='Wind Vector')
...
plt.figure()
plt.subplot(1, 2, 1)
for ep in df_norm_error["episode"].unique()[:num_episodes]:  # Limit to first 3 episodes
    ep_length = ep_length_list[ep]
    ep_norm_error = df_norm_error[df_norm_error["episode"] == ep]
    for env_id in ep_norm_error["env_id"].unique():
        norm = ep_norm_error[ep_norm_error["env_id"] == env_id]
        plt.plot(np.arange(len(norm["norm_error_lin"][:ep_length])), norm["norm_error_lin"][:ep_length])

plt.title("norm error lin")
plt.legend()
plt.tight_layout()
...
for ep in df_norm_error["episode"].unique()[:num_episodes]:  # Limit to first 3 episodes
    ...
    for env_id in ep_norm_error["env_id"].unique():
        norm = ep_norm_error[ep_norm_error["env_id"] == env_id]
        plt.plot(np.arange(len(norm["norm_error_ang"][:ep_length])), norm["norm_error_ang"][:ep_length])

plt.title("norm error ang")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(latest_dir, "norm_error.png"))
# Energy plot
# --- Energy Data ---
...
for ep in df_aero["episode"].unique()[:num_episodes]:
    ...
    for env_id in ep_aero["env_id"].unique():
        ...
        true_wind_angle = env_aero["true_wind_angle"].values[:max_timestep]
        ...
        if len(lift) < max_timestep:
            ...
            true_wind_angle = np.pad(true_wind_angle, (0, pad), constant_values=np.nan)
        ...
        true_wind_angle_all.append(true_wind_angle)
...
true_wind_angle_all = np.stack(true_wind_angle_all)
...
true_wind_angle_mean = np.nanmean(true_wind_angle_all, axis=0)
true_wind_angle_std = np.nanstd(true_wind_angle_all, axis=0)
...
# True wind Angle
plt.plot(timesteps, true_wind_angle_mean, label="True Wind Angle Mean", color='purple')
plt.fill_between(timesteps, true_wind_angle_mean - true_wind_angle_std, true_wind_angle_mean + true_wind_angle_std, alpha=0.3, 
                 color='purple', label="True Wind Angle ±1 std")
# Sail Angle
