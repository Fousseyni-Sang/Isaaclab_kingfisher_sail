...
class EpisodeMetricsPublisher(Node):
    def __init__(self, num_envs, device):
        super().__init__('episode_metrics_publisher')
        
        self.num_envs = num_envs
        self.device = device
        
        # Define the episode sums dictionary
        self._episode_sums = {
        ...
        # Create publishers for each key
        self.publishers = {
            key: self.create_publisher(Float32, f"/episode_metrics/{key}", 10)
            for key in self._episode_sums.keys()
        }

    def publish_metrics(self):
        # Calculate the averages for each metric and publish
        for key, values in self._episode_sums.items():
            avg_value = torch.mean(values).item()  # Convert to Python float
            self.get_logger().info(f"Publishing {key}: {avg_value}")
            ...
            # Optionally reset the sum after publishing
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Update your log with other metrics
