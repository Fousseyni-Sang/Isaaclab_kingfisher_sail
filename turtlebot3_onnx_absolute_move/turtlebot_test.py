...
class Turtlebot3AbsoluteMove(Node):
    ...
    def __init__(self):
        ...
        onnx_path = "/home/GTL/fsangare/Isaaclab_kingfisher_sail/logs/rl_games/turtlebot3_maneuver/2026-05-16_15-58-15/nn/turtlebot3_maneuver.onnx"
        ...
        self.test_actions = np.arange(0.82, 1.0, 0.01)
        print(f"test actions: {self.test_actions}")
        self.current_test_idx = 0
        self.test_vel_actions = 0.0

        self.test_counter = 0
        self.test_update_frquency = 40

        self.velocity_smoother = KobukiVelocitySmoother(speed_lim_v=0.5, speed_lim_w=5.4, accel_lim_v=0.3, accel_lim_w=3.5)
        ...
        self.evaluation_context_set = np.arange(0.2, 1.0, 0.5)  # example context values from 0 to 1
    ...
    def timer_callback(self):
        ...
        smooth_actions = self.velocity_smoother.step(policy_actions.abs())
        actions = smooth_actions.copy()
        v = actions[0, 0]  # forward/backward
        w = actions[0, 1]  # left/right turn

        #print(f"Action from ONNX model: v={actions[0, 0]:.2f}, w={actions[0, 1]:.2f}")

        
        #self.cmd_vel.angular.z = max(max_angular_speed, float(w)) if w > 0 else min(-max_angular_speed, float(w))
        #self.cmd_vel.linear.x = max(max_linear_speed, float(v)) if v > 0 else min(-max_linear_speed, float(v))

        """self.cmd_vel.angular.z = max(-self.max_angular_speed, min(self.max_angular_speed, float(w)))
        self.cmd_vel.linear.x  = max(-self.max_linear_speed,  min(self.max_linear_speed,  float(v)))
        """
        test_v = self.test_actions[self.current_test_idx]
        self.cmd_vel.angular.z = 0.0
        self.cmd_vel.linear.x = test_v

        if self.test_counter%self.test_update_frquency==0:
            self.current_test_idx = (self.current_test_idx + 1)%len(self.test_actions)

        self.test_counter += 1

        self.get_logger().info(
            f'current speed: {test_v:.2f}'
        )
